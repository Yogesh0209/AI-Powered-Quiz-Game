...
public class QuestionBank {
    ...
    // Each question: {Question, Option1, Option2, Option3, Option4, CorrectAnswer}
    ...
    // Returns 'num' random questions without repeating
    public static String[][] getRandomQuestions(int num) {

        String[][] selected = new String[num][6];
        ...
        for (int i = 0; i < num; i++) {
            ...
            while (used[index] == true) {
                index = rand.nextInt(allQuestions.length);
            }
            used[index] = true;

            for (int j = 0; j < 6; j++) {
                selected[i][j] = allQuestions[index][j];
            }
        }
        return selected;
    }
